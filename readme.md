...
*Słowniczek:*
*Cache - kurewsko szybka i kurewsko droga pamięć*
*Hit or Miss ratio - Skuteczność Cache - im częściej szukana w RAMie komórka znajduje się już w cache'u, tym lepiej* TODO
...
![Direct Mapping Cache](obrazki/direct_mapping_cache.png)
...
![Fully Associative Cache](obrazki/fully_associative_cache.png)
...
![Set Associative Cache](obrazki/set_associative_cache.png)
...
* Zapamiętuje stan procesora poprzez przeniesienie rejestrów PSW na stos 
* Wykonywanie procedury obsługi znajdującej się pod konkretnym adresem(zależnym od urządzenia które zgłosiło przerwanie)
### 2. Omów cykl zapisu i odczytu DRAMu
* Cykl odczytu - Pierwszą fazą żądania odczytu z pamięci stanowi pobranie adresu wiersza, w którym znajduje się komórka, a następnie zatwierdzenie tego adresu po ustabilizowaniu się stanu szyny adresowej sygnałem RAS (Row Address Strobe). Następnie na szynę adresową pamięci podawany jest adres kolumny zawierającej żądaną komórkę, który zatwierdzany jest sygnałem CAS (Column Address Strobe). Odstęp czasu pomiędzy sygnałami CAS i RAS wynika z konstrukcji pamięci i musi zapewnić czas nie tylko na ustabilizowanie się stanu szyny adresowej, lecz także na wysterowanie wiersza do odczytu.
...
* transmisja synchroniczna - częstotliwość zegara k razy większa od częstotliwości nadawania, ilość znaków synchronizacji to 1 w przypadku MonoSync lub 2 dla BiSync. Są one wysyłane przez nadawcę przed danymi w celu zsynchronizowania się urządzeń. Synchronizacja może być zewnętrzna lub wewnętrzna.
...
Przykładowo, gdy chcemy z pamięci wysłać coś przez modem, procesor musiałby pobrać te dane z pamięci i wysłać do modemu. DMA robi to za niego, dzięki czemu procesor może zająć się w tym momencie czymś innym.
...
* **Wykradanie taktów** - Procesor daje DMA dostęp do magistrali danych, kiedy chce. DMA musi czekać, dopóki procesor nie da mu zielonego światła
...
### 2. Odczyt ROM
Pierwszą fazą jest ustawienie adresu sczytanego w strukturze pamięci (linie adresowe). Uaktywnienie linii MEMREQ na podstawie adresu, a także linii CE (logicznego wyboru danego układu pamiętającego) – stanem niskim. Następnie uaktywnia się linię odczytu i pobiera dane.
